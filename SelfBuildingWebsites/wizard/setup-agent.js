...
export class SetupAgent {
  constructor(ui) {
    this.ui = ui; // { addMessage, addInput, addButtons, addProgress, setInputVisible, clearInput }
...
This takes 2-5 minutes. Ready?`);
  ...
  async doBuildRunning() {
    ...
    try {
      ...
      this.ui.addMessage('agent', `Something went wrong: <strong>${err.message}</strong>. You can retry from the admin panel once the site deploys.`);
    }
  }

...
**It identifies where controls mask exposure.** One of the most productive exercises in this process is mapping inherent-residual gaps against control effectiveness evidence. Risks where the gap is large but the control evidence is weak — effectiveness ratings based on design documentation rather than operational testing — should be systematically flagged for deeper review. These often turn out to be risks where the institution is running significantly more exposure than the residual score suggests.
